...
router.post('/registrar_cuenta', (req, res, next) => {
  if(req.body.id == null || req.body.username == null || req.body.correo == null || req.body.password == null)
  {
    res.status(500).json({status:'error'});
  }
  else
...
router.get('/ultima_cuenta', (req, res, next) => {
  db.query(
    'SELECT MAX(id_cuenta) AS id FROM CUENTA',
    (error, results) => {
      if(error)
      {
        console.error(error);
